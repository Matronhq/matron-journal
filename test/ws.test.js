...
// peer_message is NOT agent-publishable. A
...
test('op:peer_message same-account happy path — server-derived attribution, one event', async (t) => {
  ...
  upsertConversation(s.db, { id: 'from', ownerUserId: dan.id, agentDeviceId: agA.deviceId, title: 'Sender Session' })
  ...
  assert.equal(p.from_kind, null)             // from_kind degrades to null upstream (no agent_kind column); bogus msg kind ignored
