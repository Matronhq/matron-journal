...
export async function handleOp({ db, hub, conn, msg, pushPipeline = noopPushPipeline, toolStreams, statusCache = makeStatusCache(), rpcMaxBytes = RPC_MAX_BYTES, frameBytes = 0, broker, spawnFoldersTimeoutMs = 4000, waker = null, log = console.log }) {
  ...
  try {
    switch (msg.op) {
      ...
      case 'peer_message': {
        ...
        const fromConvo = db.prepare(
          'SELECT owner_user_id, agent_device_id, title FROM conversations WHERE id=?'
        ).get(msg.from_convo)
        if (!fromConvo || fromConvo.owner_user_id !== conn.userId
          || fromConvo.agent_device_id !== conn.deviceId) {
        ...
        const appendArgs = {
          ...
          payload: {
            ...
            // from_kind (sender backend: claude|codex) is derived from the
            // fork-only conversations.agent_kind column, which upstream does
            // not have. Degrade to null here; receivers already treat a null
            // kind as "unknown" and omit the label. When agent_kind lands
            // upstream this reads fromConvo.agent_kind again.
            from_kind: null,
            body,
          },
        }
      ...
      case 'publish': {
        if (conn.kind !== 'agent') return fail('forbidden')
        // peer_message is server-authoritative
