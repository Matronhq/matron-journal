// Server-side path-jail for the Matron File Explorer API (reads and writes).
//
...
export function denialToStatus(reason) {
  ...
  // Write conflicts. Each names a state the caller can resolve by
...
// single-operator threat model (the same class as the accepted hard-link
// residual on the read side). Node also lacks renameat2(RENAME_NOREPLACE), so regular-file moves
...
// act, so the caller has to say so explicitly (a server-enforced confirm). When it is allowed, the previous content is copied into the
