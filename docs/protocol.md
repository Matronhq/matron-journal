...
**While the ask is pending, the item is journal-owned.** Pending means `awaiting_user` **or** `approved` — the lock must hold through the window between the tap and the target's `start` reply. Every agent mutation of it through the item routes — `PATCH`, `close`, `reopen`, `comments`, `rank` — answers `403 {error:'forbidden'}` (`isPendingConsentMirror`, `src/items.js`; the guard sits in `handleItemsRoute`): the asking agent, if prompt-injected, must not rewrite the task the user reads there or close it out of the open list. Reads are unchanged, a client's hand-close is still the user's own call, and once the row resolves the item is an ordinary item again.
