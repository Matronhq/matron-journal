...
- **[CENTRAL]** — the journal server host, dev-2 (`systemd
  matron-journal.service`, DB `/home/danbarker/matron-journal/data/matron.db`,
  bound `127.0.0.1:9810`, public `https://chat.yearbooks.be`).
- **[BOX:&lt;host&gt;]** — a teammate's dev box running `claude-matrix-bridge`.
- **[CHEF]** — the `yearbook-infra` repo (`dev_server` cookbook). PROPOSED
...
- One journal server process on dev-2 (`matron-journal.service`), listening
  `127.0.0.1:9810`, published at `https://chat.yearbooks.be` through a
...
- **No Cloudflare Access** on `chat.yearbooks.be` by design — native-client
...
```
# [CENTRAL]
cd /home/danbarker/matron-journal
export MATRON_DB=/home/danbarker/matron-journal/data/matron.db
...
- The teammate changes it on first login via `POST /password` (Bearer, client
  ...
  curl -fsS https://chat.yearbooks.be/password \
...
# [CENTRAL]
cd /home/danbarker/matron-journal
MATRON_DB=/home/danbarker/matron-journal/data/matron.db \
...
JOURNAL_WS_URL=wss://chat.yearbooks.be/ws
...
- The media HTTP endpoint is **derived** from `JOURNAL_WS_URL`
  ...
  token — no extra var. So `wss://chat.yearbooks.be/ws` implies media at
...
## 4. Chef automation — PROPOSED (yearbook-infra `dev_server` cookbook)

**[CHEF] — this section is a proposal; the change lands in `yearbook-infra`,
...
- Template the bridge journal env into `~/claude-matrix-bridge/.env`:
  `JOURNAL_WS_URL=wss://chat.yearbooks.be/ws`, `JOURNAL_TOKEN_FILE`,
...
- **OPEN (Chef):** exact data-bag key layout for a per-host agent token, and
  ...
  `yearbook-infra` review. Do **not** invent a key path.
...
- Log in against `https://chat.yearbooks.be`: the app performs
...
- [ ] **Snapshot shows convos — client / [CENTRAL].** `GET /snapshot` (Bearer)
      ...
      ```
      curl -fsS https://chat.yearbooks.be/snapshot -H "Authorization: Bearer <token>"
      ```
...
- [ ] **Lag sane — [CENTRAL].** `/metrics` (or `matron-admin status`) shows the
      ...
      ```
      # [CENTRAL]
      MATRON_DB=/home/danbarker/matron-journal/data/matron.db npx matron-admin status
      ```
...
```
MATRON_DB=/home/danbarker/matron-journal/data/matron.db npx matron-admin device list <username>
MATRON_DB=/home/danbarker/matron-journal/data/matron.db npx matron-admin device revoke <device_id>
```

**Lost password reset — [CENTRAL].**
```
MATRON_DB=/home/danbarker/matron-journal/data/matron.db npx matron-admin user passwd <username> --password '<new-password>'
```
...
```
MATRON_DB=/home/danbarker/matron-journal/data/matron.db npx matron-admin offload [--days N]
```
...
- DB: `MATRON_DB` = `/home/danbarker/matron-journal/data/matron.db` (committed
...
- Blobs: `MATRON_MEDIA_DIR`, or its default `<dirname of the DB>/media`
  (README, `src/media.js`), i.e. `/home/danbarker/matron-journal/data/media`
...
- **TLS-only public edge.** The only public exposure is
  `https://chat.yearbooks.be` via the Cloudflare tunnel; the server itself
