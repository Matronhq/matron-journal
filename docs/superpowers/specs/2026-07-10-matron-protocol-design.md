...
The server sends APNs HTTP/2 pushes directly using the existing `.p8` signing key (team `4LJ7WRRRFD`, topic `chat.matron.x`) — sygnal is retired. Clients register `apns_token` on their device row after enrollment.
