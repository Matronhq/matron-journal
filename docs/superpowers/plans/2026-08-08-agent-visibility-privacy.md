...
Task 5's per-op private-target checks (`agent_invite`'s target lookup, `agent_join`'s owner check) turned out to leave the other three room ops with distinct error shapes for a private-owned room, so during execution the gate moved into `loadRoom` — the single lookup shared by all five room ops — with an `isKnownParticipant` exemption (initiator-self, actually-delivered, or joined; a merely parked `awaiting_user` or `denied` row does not exempt) so one choke point closes all five without leaking existence through the ones this plan didn't individually specify. Separately, a Task 8 covering `GET /snapshot` was added after this plan was written, by the search branch's final security review — `/snapshot` predates this feature and needed its own two rules (snippet omitted for every agent caller; private-owned conversations excluded for a filtered agent caller) that this plan's four-surface scope didn't originally cover. See `docs/superpowers/specs/2026-08-07-agent-visibility-privacy-design.md`'s "Implementation deltas" section for the full explanation of both.
