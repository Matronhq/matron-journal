...
| `MATRON_FILE_READ_ROOTS` | unset (file API off) | Colon-separated directories the File Explorer API may browse and read (`/files/list`, `/files/meta`, `/files/content`). Unset or empty keeps every `/files/*` route a 404. Only a journal admin's client devices may use it (`matron-admin user admin <name> on`). Credential/config paths (`.ssh`, `.env*`, `.aws`, …) and the journal's own state (database, media, audit log) are always denied. Linux only (needs `/proc/self/fd`). Under the shipped systemd unit, `ProtectHome=yes` hides `/home` roots |
...
| `MATRON_FILE_WRITE_ROOTS` | unset | Colon-separated write roots. Each must sit inside a read root and may not overlap the database, preapprove key, media dir or audit log, or be a broad system directory (`/`, `/root`, `/home`, `/etc`, `/usr`, `/var`), and must be writable by the process (under the shipped systemd unit, add them to `ReadWritePaths`); a bad value fails the boot |
